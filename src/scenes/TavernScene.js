...
export class TavernScene extends Phaser.Scene {
  ...
  render(message = '') {
    ...
    const cards = COMPANIONS.map((ally) => {
      ...
      const progressText = bond?.levelCappedByPlayer ? '(용사 레벨 필요)' : bond?.xpToNext ? `(${bond.xp}/${bond.xpToNext})` : '(MAX)';
      const bondLine = bond
        ? `<small>유대 Lv.${bond.level} ${progressText} · 공격 ${bond.attack} · 방어 ${bond.defense}${bond.awakenings ? ` · 각성 ${bond.awakenings}단계 (위력 +${Math.round((bond.abilityMultiplier - 1) * 100)}%)` : ''}</small>`
